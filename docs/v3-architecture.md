...
| 컴포넌트 | 파일 | 역할 |
|---------|------|------|
| Main Process | `main.js` (1266줄) | Electron 메인, HTTP 훅 서버, Liveness Checker |
| Renderer | `renderer.js` (1117줄) | 픽셀 아바타 애니메이션, 그리드 레이아웃 |
| Hook Script | `hook.js` (47줄) | Claude CLI → HTTP POST 브릿지 |
| Agent Manager | `agentManager.js` (218줄) | 에이전트 상태 관리, 이벤트 에밋 |
| Dashboard Server | `dashboard-server.js` (401줄) | REST API + WebSocket 대시보드 |
...
Claude CLI ──hook.js──▶ HTTP POST(:47821) ──▶ main.js:processHookEvent()
                            ...
                            agentManager       dashboard-server
                                │                    │
                                ▼                    ▼
                          renderer.js          dashboard.html
                        (픽셀 아바타)          (웹 대시보드)
```

### 1.2 참고 프로젝트: Mission Control (reffer/)

Mission Control은 **Next.js 16 + SQLite + Zustand** 기반의 풀스펙 에이전트 오케스트레이션 대시보드다.

**우리가 배울 핵심 기능:**

| 기능 | Mission Control 구현 | 우리 프로젝트 적용 |
|------|---------------------|-------------------|
| **Claude 세션 스캔** | `claude-sessions.ts` — `~/.claude/projects/` JSONL 파싱 | transcript_path 활용으로 세션 분석 |
| **실시간 이벤트** | SSE (`/api/events`) + EventBus | WebSocket → SSE 전환 고려 |
| **오피스 시각화** | `office-panel.tsx` — 2D 타일맵, A* 경로탐색, 스프라이트 애니메이션 | 아바타 시각화 고도화 참고 |
| **토큰/비용 추적** | `token_usage` 테이블, Recharts 차트 | PostToolUse에서 token_usage 추출 |
| **상태 관리** | Zustand store (796줄, 40+ 액션) | 현재 EventEmitter → 구조화 |
...
## 2. 현재 코드의 문제점 & 개선 필요사항

### 2.1 훅 데이터 처리 문제

```javascript
// ❌ 현재 main.js:startHookServer() — 스키마 오류
const hookSchema = {
  properties: {
    state: { type: 'string' },    // ❌ Claude 훅에 'state' 필드 없음
    tool: { type: 'string' },     // ❌ 'tool_name'이 올바른 필드명
    // transcript_path 누락       // ❌ 모든 훅에 포함된 핵심 필드
    // permission_mode 누락       // ❌ 모든 훅에 포함
  },
  additionalProperties: true      // ❌ 너무 관대
};
...
const hookSchema = {
  ...
  properties: {
    hook_event_name: { type: 'string', enum: [...모든 이벤트] },
    session_id: { type: 'string' },
    transcript_path: { type: 'string' },
    cwd: { type: 'string' },
    permission_mode: { type: 'string' },
    tool_name: { type: 'string' },      // 'tool' → 'tool_name'
    tool_input: { type: 'object' },
    source: { type: 'string' },          // SessionStart용
    model: { type: 'string' },           // SessionStart용
  }
};
```

### 2.2 transcript_path 미활용

```javascript
// ❌ 현재: transcript_path를 null로 무시
agentManager.updateAgent({
  sessionId, projectPath: cwd,
  jsonlPath: null,  // 🔴 transcript_path 버림
});

// ✅ 수정안: transcript_path 저장 후 세션 분석에 활용
agentManager.updateAgent({
  sessionId, projectPath: cwd,
  jsonlPath: data.transcript_path,  // JSONL 세션 로그 경로
  model: data.model,
...
// Claude CLI는 항상 session_id (snake_case) 사용
// JSONL 내부는 sessionId (camelCase) 사용 → 불일치 가능

// ✅ 수정안: Claude 표준(session_id) 우선, 매핑 테이블 관리
```
